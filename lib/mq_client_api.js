...
class MQClientAPI extends RemotingClient {
  ...
  async sendMessage(brokerAddr, brokerName, msg, requestHeader, timeoutMillis) {
    ...
    return {
      sendStatus,
      msgId: MessageClientIDSetter.getUniqID(msg),
      offsetMsgId: responseHeader.msgId,
      messageQueue,
  ...
  async lockBatchMQ(brokerAddr, consumerGroup, clientId, mqSet, timeoutMillis) {
    const requestBody = {
      consumerGroup,
      clientId,
      mqSet,
    ...
    request.body = new Buffer(JSON.stringify(requestBody));
    const response = await this.invoke(brokerAddr, request, timeoutMillis);
    switch (response.code) {
      case ResponseCode.SUCCESS:
        if (response.body) {
          const body = JSON2.parse(response.body.toString());
          return body.lockOKMQSet.map(mq => new MessageQueue(mq.topic, mq.brokerName, mq.queueId));
        }
        break;
      default:
        this._defaultHandler(request, response);
  ...
  async unlockBatchMQ(brokerAddr, consumerGroup, clientId, mqSet, timeoutMillis) {
    const requestBody = {
      consumerGroup,
      clientId,
      mqSet,
  ...
  async endTransactionOneway(addr, requestHeader, remark, timeoutMillis) {
    const request = RemotingCommand.createRequestCommand(RequestCode.END_TRANSACTION, requestHeader);

    request.remark = remark;
    await this.invokeOneway(addr, request, timeoutMillis);
  }
