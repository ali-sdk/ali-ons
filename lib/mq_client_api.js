...
class MQClientAPI extends RemotingClient {
  ...
  async getTopicRouteInfoFromNameServer(topic, timeoutMillis) {
    ...
    let response = {};
    let success = false;
    const tempNamesrvAddrList = this._namesrvAddrList.slice(0);
    while (!success && tempNamesrvAddrList.length > 0) {
      const index = Math.floor(Math.random() * tempNamesrvAddrList.length);
      const addr = tempNamesrvAddrList[index];
      try {
        response = await this.invoke(addr, request, timeoutMillis);
        success = true;
      } catch (e) {
        tempNamesrvAddrList.splice(index, 1);
      }
    }
    if (!success) {
      const err = new Error();
      err.message = `[mq:api] name sever error:${this._namesrvAddrList.join(',')}`;
      this.logger.error(err);
      return null;
    }
    switch (response.code) {
