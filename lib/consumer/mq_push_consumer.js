...
class MQPushConsumer extends ClientConfig {
  ...
  async unlock(messageQueue, delay) {
    ...
    try {
      ...
      this.logger.info('unlockBatchMQ ok, group %s, mq %s', this.consumerGroup, messageQueue.key);
    } catch (e) {
      this.logger.error('unlockBatchMQ exception, %s', messageQueue.key);
    }
  }
  ...
  // orderlyProcessChain里存有处理顺序消息的Promise链, 内存消耗最小且不会泄露(内部任务有超时机制)
  submitConsumeRequest(messageQueue, processQueue, checkDispathToConsume, args) {
    ...
    const task = prevTask.then(async () => {
      try {
        await this._consumeMessageQueueOrderly(messageQueue, args);
      } catch (e) {
