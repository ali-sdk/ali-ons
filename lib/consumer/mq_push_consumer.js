...
class MQPushConsumer extends ClientConfig {
  ...
  subscribe(topic, subExpression, handler) {
    ...
    if (this.namespace && !topic.includes(this.namespace)) {
      realTopic = `${this.namespace}%${realTopic}`;
    }

    assert(is.asyncFunction(handler), '[MQPushConsumer] handler should be a asyncFunction');
    assert(!this.subscriptions.has(realTopic), `[MQPushConsumer] ONLY one handler allowed for topic=${realTopic}`);

    const subscriptionData = this.buildSubscriptionData(this.consumerGroup, realTopic, subExpression);
    const tagsSet = subscriptionData.tagsSet;
    const needFilter = !!tagsSet.length;
    this.subscriptions.set(realTopic, {
      handler,
      subscriptionData,
    });
    ...
    (async () => {
      try {
        ...
        if (!this._topicSubscribeInfoTable.has(realTopic)) {
        ...
        // 消息消费循环
        while (!this._isClosed && this.subscriptions.has(realTopic)) {
          await this._consumeMessageLoop(realTopic, needFilter, tagsSet, subExpression);
        }
