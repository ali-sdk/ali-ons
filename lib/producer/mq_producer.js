...
const MessageClientIDSetter = require('../message/message_client_id_setter');
const LocalTransactionState = require('./local_transaction_state');
...
class MQProducer extends ClientConfig {
   ...
   * @param {Function/String} selector - select method
   * @return {Object} sendResult
   */
  async send(msg, selector) {
    await this.ready();
    ...
    if (typeof selector === 'function') {
      ...
      try {
        // 克隆messgeQueueList以防止被用户破坏
        const messgeQueueList = topicPublishInfo.messageQueueList.map(e => Object.assign({}, e));
        messageQueue = selector(messgeQueueList);
      } catch (e) {
        throw new Error('select message queue throwed exception.');
      }
      if (!messageQueue) {
        throw new Error('select message queue return null.', null);
      }
      // 从用户选择的messageQueue还原为内部的messageQueue, 因为用户给的结果是不可靠的
      messageQueue = topicPublishInfo.messageQueueList.find(mq => messageQueue.key === mq.key);
  ...
  async sendMessageInTransaction(msg, arg) {
    if (!(this.options.transaction && this.options.transaction.executeLocalTransaction)) {
      throw new Error('transaction not enabled or executeLocalTransaction is null');
    }
    ...
    try {
      ...
      throw new Error('send message Exception', e);
    }

    let localTransactionState = LocalTransactionState.UNKNOW;
    let localException = null;
    switch (sendResult.sendStatus) {
      case SendStatus.SEND_OK:
        try {
          if (sendResult.transactionId) {
            msg.putUserProperty('__transactionId__', sendResult.transactionId);
          }
          const transactionId = msg.properties[MessageConst.PROPERTY_UNIQ_CLIENT_MESSAGE_ID_KEYIDX];
          if (transactionId) {
            msg.transactionId = transactionId;
          }
          if (this.options.transaction && this.options.transaction.executeLocalTransaction) {
            this.logger.debug('Used new transaction API');
            localTransactionState = this.options.transaction.executeLocalTransaction(msg, arg, LocalTransactionState);
          }
          ...
          if (localTransactionState !== LocalTransactionState.COMMIT_MESSAGE) {
            this.logger.info('executeLocalTransactionBranch return %s', localTransactionState);
            this.logger.info(msg.toString());
          }
        } catch (e) {
          this.logger.info('executeLocalTransactionBranch exception');
          this.logger.info(msg.toString());
          localException = e;
        }
        break;
    ...
    try {
      ...
      this.logger.warn('local transaction execute ' + localTransactionState + ', but end broker transaction failed');
    }

    const transactionSendResult = Object.assign({}, sendResult);
    transactionSendResult.localTransactionState = localTransactionState;
    return transactionSendResult;
  }

  async checkTransactionState(addr, message, header) {
    let localTransactionState = LocalTransactionState.UNKNOW;
    let exception = null;
    if (this.options.transaction && this.options.transaction.checkLocalTransaction) {
      this.logger.debug('Used new check API in transaction message');
      try {
        localTransactionState = this.options.transaction.checkLocalTransaction(message, LocalTransactionState);
      } catch (e) {
        this.logger.error('Broker call checkTransactionState, but checkLocalTransactionState exception');
        exception = e;
      }
    }
    const thisHeader = {};
    thisHeader.commitLogOffset = Number(header.commitLogOffset.toString());
    thisHeader.producerGroup = this.producerGroup;
    thisHeader.tranStateTableOffset = header.tranStateTableOffset;
    thisHeader.fromTransactionCheck = true;

    let uniqueKey = message.properties[MessageConst.PROPERTY_UNIQ_CLIENT_MESSAGE_ID_KEYIDX];
    if (!uniqueKey) {
      uniqueKey = message.msgId;
    }
    thisHeader.msgId = uniqueKey;
    thisHeader.transactionId = header.transactionId;
    switch (localTransactionState) {
      ...
      case LocalTransactionState.ROLLBACK_MESSAGE:
        ...
        this.logger.warn('when broker check, client rollback this transaction');
        break;
      case LocalTransactionState.UNKNOW:
        thisHeader.commitOrRollback = MessageSysFlag.TransactionNotType;
        this.logger.warn('when broker check, client does not know this transaction state', message.body.toString());
        break;
      default:
        break;
    }
    ...
    try {
      await this._mqClient.endTransactionOneway(addr, thisHeader, remark, 3000);
    } catch (e) {
      this.logger.error('endTransactionOneway exception');
    }
  }

  async endTransaction(sendResult, localTransactionState, localException) {
    let id;
    if (sendResult.offsetMsgId) {
    ...
    const requestHeader = {
      transactionId,
      commitLogOffset: Number(id.offset.toString()),
    };
    switch (localTransactionState) {
      case LocalTransactionState.COMMIT_MESSAGE:
    ...
    await this._mqClient.endTransactionOneway(brokerAddr, requestHeader, remark, this.options.sendMsgTimeout);
  }
