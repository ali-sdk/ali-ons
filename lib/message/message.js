...
class Message {
  ...
  putUserProperty(name, value) {
    if (MessageConst.STRING_HASH_SET.has(name)) {
      throw new Error(`The Property<${name}> is used by system, input another please`);
    }

    if (!(typeof name === 'string' && name.trim() && typeof value === 'string' && value.trim())) {
      throw new Error('The name or value of property can not be null or blank string!');
    }
